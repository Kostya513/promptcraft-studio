...
export function AccountsList({ showFormProp, onFormClose }: AccountsListProps) {
  ...
  // if parent signals to open form, react
  useEffect(() => {
    if (showFormProp && !showForm) {
      setShowForm(true);
    }
  ...
  // openAdd is used by toolbar button and can also be triggered by parent via prop
  const closeForm = () => {
    setShowForm(false);
    onFormClose?.();
  };
  ...
  const openAdd = () => {
    setForm({ service: "", url: "", profile: "Личный", type: "Аккаунт", status: "Активен", plan: "", costAmount: "", costPeriod: "мес", regDate: "", nextPayment: "", notes: "", login: "" });
    setShowForm(true);
  };
  ...
  const handleSave = () => {
    ...
    const newAcc: Account = {
      ...
      has2fa: false, lastPwChange: "", login: form.login || "", postingEnabled: false, lastLogin: "",
    };
  ...
  const handleSaveGenerated = (profile: { name: string; email: string; password: string }) => {
    const newAcc: Account = {
      ...
      has2fa: false, lastPwChange: new Date().toISOString().split("T")[0], login: profile.email,
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      ...
      {filtered.length === 0 ? (
        ...
        <>
          {/* existing table and cards markup below, unchanged */}
